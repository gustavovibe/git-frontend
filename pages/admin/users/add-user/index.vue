        ...
        <div class="grid">
            ...
            <div class="col-12 lg:col-10">
                <div class="back-green-vibe-light px-5">
                    <h1>{{ indicador?'Edit User':'Add User' }}</h1>
                </div>
                <div>
                    <div class="card lg:flex justify-content-between max-80-lg">
                        ...
                        <div class="flex flex-column gap-2">
                            ...
                            <InputText id="username" v-model="paramsSent.email" aria-describedby="username-help" />
                        ...
                        <div class="flex flex-column gap-2">
                            ...
                            <div class="flex">
                                <Dropdown v-model="paramsSent.phone_country" :options="phoneLADA" optionLabel="name" optionValue="name" placeholder="(+52)" class="w-8rem mr-3" style="height: 40px;"/>
                ...
                <div>
                    <p class="canaro-font text-xl font-semibold">Permissions</p>
                    <p class="interstate-font text-lg font-light">Select the tabs this user can access and edit.</p>
										<div class="container">
											<div v-for="permission in permissions_vals" :key="permission.text" class="permission-item" >
												<div class="flex align-items-center">
													<Checkbox  v-model="permissionsSent[permission.key]"  :binary="true"  />
													<label  class="ml-2">{{ permission.text }}</label>
                        </div>
											</div>
										</div>
                </div>
                <div>
					...
					<div class="container">
							...
							<br>
						<!-- 	<div>
							  <div v-if="indicador" class="flex flex-column gap-2" style="width: 25%;">
								<label  class="canaro-font font-medium text-lg">Password</label>
								<Password :id="passwordId" v-model="paramsSent.code"  toggleMask  />
							</div>
							</div> -->
                <div>
...
<script setup>
	...
	const deletef = async(queryParams)=>{
		try{
			loading.value = true;
			const res = await fetch(`${config.public.BACKEND_URL}/api/delete-users?user_id=${paramsSent.value.id}`,{
				method: 'DELETE',
			});
	...
	const  debounceValidateEmail=EmailsVals;
	...
	watch(()=>paramsSent.value.email,debounceValidateEmail);

	onMounted(() => {
		getJobs();
		clearForm();
		if(indicador.value === 1 && user.value){
			paramsSent.value = { ...user.value };
			getUsers(paramsSent.value.id);
