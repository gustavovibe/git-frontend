...
const ages = computed(() => {
  ...
  const isSpecialPage = path.startsWith('/tour') || path.startsWith('/book')

  if (!isSpecialPage) {
    // default ages when not in /tour or /book
    return range(1, 17).map(age => ({ label: `${age} years`, value: age }))
  }

  // same logic you already have
  const resp = prices.value
  if (!resp) return []
