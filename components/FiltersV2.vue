<template>
  <div id="filters">
   <div class="flex" style="justify-content: space-between;margin-bottom: 10px;">
    ...
    <div class="filter" v-if="durationSliderRange?.[0]">
      <Accordion :activeIndex="0">
        <AccordionTab :class="filter - tab">
          ...
          <div class="input-container" style="text-align:center" >
            <Slider :min="durationSliderRange[0]" :max="durationSliderRange[1]" v-model="timeSlider" @slideend="timeFilter" range
    ...
    <div class="filter" v-if="sliderPrice">
      <Accordion :activeIndex="0">
        <AccordionTab :class="filter - tab">
          <template #header>
            <span>
    ...
    <div class="filter" v-if="ageSlider?.[0]">
      <Accordion :activeIndex="0">
        <AccordionTab :class="filter - tab">
          <template #header>
            <span>
    ...
    <div class="filter" v-if="groupSlider?.[0]">
      <Accordion :activeIndex="0">
        <AccordionTab :class="filter - tab">
          <template #header>
            <span>
...
const props = defineProps({
  types: { type: Array, default: () => [] },  // for categories/styles/ratings
  duration: { type: Array, default: null },  // [min,max] or null
  ages: { type: Array, default: null },      // [min,max] or null
  group: { type: Array, default: null },      // [min,max] or null
...
const languages = ref([ { name: "English", id: "1" }]);
const durationSliderRange = ref(props.duration);
...
const timeSlider = ref(Array.isArray(durationSliderRange.value) && durationSliderRange.value.length ? [...durationSliderRange.value] : [2, 65]);
const ageSlider = ref( props.ages ? props.ages : [2, 99]);
const groupSlider = ref(props.group ? props.group : [4, 50]);
const priceRange = ref([]);
	
// helper to split types into guides, ratings and styles	
function processTypes(types) {
  console.log("processTypes",types);	
  categories.value = [];
  styles.value = [];
  ratings.value = [];
  ...
  types.forEach(type => {
    const id = Number(type.key); // cast to number for reliable switch
    switch (id) {
      case 373:
      ...
      // NOTE: corrected duplicate case: Hiking & Trekking should be case 73
      case 381:
      ...
      case 56:
        ...
        break;
      default:
        // console.debug('unknown type id', id);
        break;
    }
  });
}
...
function changingPrices(newValue) {
  ...
  if (newValue === 'Per Day') {
    ...
    sliderMax.value = 900;  
    sliderStep.value = 20;
    sliderPrice.value = [100,900];
  } else {
    sliderMin.value = 300;
    sliderMax.value = 30000;
...
const searchItems = (event) => {
  ...
  if(query === 'all') {
    filteredItems.value = cities.value
    .map(tour => ({
...
//watchers to change values when load more tours	
watch(() => props.citiesSelected, (cities) => {
  console.log("global cities", cities);
});

watch(() => props.durationSliderRange, (newRange) => {
  if (!newRange || !newRange.length) return;
  timeSlider.value = [...newRange];
});
	
watch(() => props.pricesSelected, (newPrices) => {
...
function applyFilters() {
  // emit a full payload for convenience; parent can ignore if it uses v-models
  const payload = {
    selectedTime: timeSlider.value,
    selectedPrice: priceRange.value,
    selectedAges: ageSlider.value,
    selectedGroup: groupSlider.value,
    selectedCategories: selectedCategories.value,
    selectedRatings: selectedratings.value,
    selectedStyles: selectedStyles.value,
    selectedCities: selectingCities.value
  };
  console.log("applyFilters@ from component",payload);	
  emit('apply', payload);
}
