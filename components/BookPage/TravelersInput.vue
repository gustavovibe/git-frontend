...
const ages = ref();		
ages.value = await calculateAges(props.prices);	
console.log("ages.value",ages.value);	
...
async function calculateAges(prices){
  // same logic you already have
  const resp = prices;
  console.log("prices on calculateAges",resp);	
  if (!resp) return []
